...
if (isset($_GET['one_post'])) {
  ...
  try {
    $stmt = $db->prepare("SELECT postid,post,description,post_date,posts.userid,username FROM posts INNER JOIN forum_users ON forum_users.userid = posts.userid WHERE postid = ?");
    $stmt->execute([$postid]);
  ...
  try {
    $comment_query = "SELECT comments.commentid, comments.userid, comment,comment_date,username FROM comments INNER JOIN forum_users ON forum_users.userid = comments.userid WHERE comments.postid = ?";
              ...
              <div class="col-lg-3">
                <div class="reply-details">
                  ...
                  <?php if (isset($_SESSION['userid']) && $_SESSION['userid'] == $comment['userid']) : ?>
                    <i style="color: red;" class="glyphicon glyphicon-trash"></i> <a style="color: red;" href="delete.php?remove=<?php echo $comment['commentid'];  ?>">Delete</a>
                  <?php endif; ?>
                </div>
              </div>
            </div>
