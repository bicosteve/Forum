...
if (isset($_POST['register']) == 'POST') {
  ...
  if (!isset($username_err) && !isset($email_err) && !isset($password_err) && !isset($password2_err) && !isset($match_err)) {
    try {
      $stmt = $db->prepare("SELECT email FROM forum_users WHERE email=:email");
      ...
      if (!isset($register_err)) {
        ...
        $today = date("Y-m-d");
        $query = "INSERT INTO forum_users(username,email,join_date,password) VALUES (:username,:email,:join_date,:password)";
